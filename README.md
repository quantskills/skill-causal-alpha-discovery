...
1. **生成特征池**：从 6 个 OHLCV 字段生成 60 个衍生特征
2. **因果发现**：默认 rank-IC 选择（快速），可选 PC/LiNGAM/NOTEARS
3. **构建因子**：筛选 top 因果父节点 → 映射为有效 OHLCV 表达式 →
   按 ATE 加权组合 → 截面 rank 归一化（默认最多 3 个成分）
4. **不变性检验**：在测试期划分 6 个市场体制 → χ² 齐性检验
5. **回测验证**：内置引擎计算 rank IC / 多头组合 / Sharpe / 最大回撤
6. **分析报告**：生成含信号家族分类、IC 稳定性、体制不变性的综合报告
